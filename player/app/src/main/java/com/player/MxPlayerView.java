 ...
 * during playback, and displays playback controls using a {@link MxPlayerControlView}.
 *
 * <p>A MyPlayerView can be customized by setting attributes (or calling corresponding methods),
 * overriding drawables, overriding the view's layout file, or by specifying a custom view layout
 * file.
 *
 * <h3>Attributes</h3>
 *
 * The following attributes can be set on a MyPlayerView when used in a layout XML file:
 *
 * <ul>
 ...
 *       inflated by the child {@link MxPlayerControlView}. See below for more details.
 *       <ul>
 *         <li>Corresponding method: None
 *         <li>Default: {@code R.layout.exo_player_control_view}
 *       </ul>
 *   <li>All attributes that can be set on {@link MxPlayerControlView} and {@link DefaultTimeBar} can
 *       also be set on a MyPlayerView, and will be propagated to the inflated {@link
 *       MxPlayerControlView} unless the layout is overridden to specify a custom {@code
 *       exo_controller} (see below).
 * </ul>
 *
 * <h3>Overriding drawables</h3>
 *
 * The drawables used by {@link MxPlayerControlView} (with its default layout file) can be overridden
 * by drawables with the same names defined in your application. See the {@link MxPlayerControlView}
 * documentation for a list of drawables that can be overridden.
 *
 * <h3>Overriding the layout file</h3>
 *
 * To customize the layout of MyPlayerView throughout your app, or just for certain configurations,
 ...
 * inflated for use by MyPlayerView. The view identifies and binds its children by looking for the
 * following ids:
 *
 * <p>
 *
 ...
 *       {@link MxPlayerControlView}. Ignored if an {@code exo_controller} view exists.
 *       <ul>
 *         <li>Type: {@link View}
 *       </ul>
 *   <li><b>{@code exo_controller}</b> - An already inflated {@link MxPlayerControlView}. Allows use
 *       of a custom extension of {@link MxPlayerControlView}. {@link MxPlayerControlView} and {@link
 *       DefaultTimeBar} attributes set on the MyPlayerView will not be automatically propagated
 *       through to this instance. If a view exists with this id, any {@code
 *       exo_controller_placeholder} view will be ignored.
 *       <ul>
 *         <li>Type: {@link MxPlayerControlView}
 *       </ul>
 ...
 *
 * Defining your own {@code exo_player_view.xml} is useful to customize the layout of MyPlayerView
 ...
 * MyPlayerView. This will cause the specified layout to be inflated instead of {@code
 * exo_player_view.xml} for only the instance on which the attribute is set.
 */
public class MxPlayerView extends FrameLayout implements AdsLoader.AdViewProvider {
    ...
    private final MxPlayerView.ComponentListener componentListener;
    ...
    @Nullable private final MxPlayerControlView controller;
    @Nullable private final FrameLayout adOverlayFrameLayout;
    @Nullable private final FrameLayout overlayFrameLayout;

    @Nullable private Player player;
    private boolean useController;
    @Nullable private MxPlayerControlView.VisibilityListener controllerVisibilityListener;
    private boolean useArtwork;
    @Nullable private Drawable defaultArtwork;
    private @MxPlayerView.ShowBuffering
    int showBuffering;
    ...
    private ViewGroup controllerParent = null;
    MxPlayerControlView.DisplayModeListener displayModeListener = null;

    public MxPlayerView(Context context) {
        this(context, /* attrs= */ null);
    }

    public MxPlayerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, /* defStyleAttr= */ 0);
    }

    @SuppressWarnings({"nullness:argument.type.incompatible", "nullness:method.invocation.invalid"})
    public MxPlayerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        componentListener = new MxPlayerView.ComponentListener();
        ...
        int playerLayoutId = R.layout.mx_player_view;
        boolean useArtwork = true;
        int defaultArtworkId = 0;
        boolean useController = true;
        int surfaceType = SURFACE_TYPE_SURFACE_VIEW;
        int resizeMode = AspectRatioFrameLayout.RESIZE_MODE_FIT;
        int controllerShowTimeoutMs = MxPlayerControlView.DEFAULT_SHOW_TIMEOUT_MS;
        ...
        MxPlayerControlView customController = findViewById(R.id.exo_controller);
        ...
        if (customController != null) {
            ...
            this.controller = new MxPlayerControlView(context, null, 0, attrs);
        ...
        if (controller != null) {
            ...
            controller.setDisplayModeListener(componentListener);
        }
    }
    ...
    public static void switchTargetView(
            Player player, @Nullable MxPlayerView oldPlayerView, @Nullable MxPlayerView newPlayerView) {
     ...
     * {@link #switchTargetView(Player, com.google.android.exoplayer2.ui.MyPlayerView, com.google.android.exoplayer2.ui.MyPlayerView)} rather than this method. If you do
     ...
     */
    public void setShowBuffering(@MxPlayerView.ShowBuffering int showBuffering) {
    ...
    /**
     * Set the {@link MxPlayerControlView.VisibilityListener}.
     *
     * @param listener The listener to be notified about visibility changes, or null to remove the
     *     current listener.
     */
    public void setControllerVisibilityListener(
            @Nullable MxPlayerControlView.VisibilityListener listener) {
        Assertions.checkStateNotNull(controller);
    ...
    private final class ComponentListener
            ...
            SingleTapListener,
            MxPlayerControlView.VisibilityListener,
            MxPlayerControlView.DisplayModeListener {
        ...
        @Override
        public void onDisplayModeChanged(MxPlayerControlView.DisplayMode displayMode) {
            if (MxPlayerControlView.DisplayMode.NORMAL == displayMode) {
                Activity activity = MxPlayerViewUtil.scanForActivity(getContext());
                ViewGroup contentView = activity.findViewById(android.R.id.content);

                MxPlayerViewUtil.showActionBar(getContext());
                activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);

                contentView.removeView(controller);
                contentView.removeView(contentFrame);
                addView(contentFrame, new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT, Gravity.CENTER));
                controllerParent.addView(controller);
            } else if (MxPlayerControlView.DisplayMode.FULL_SCREEN == displayMode) {
                Activity activity = MxPlayerViewUtil.scanForActivity(getContext());
                ViewGroup contentView = activity.findViewById(android.R.id.content);

                MxPlayerViewUtil.hideActionBar(getContext());
                activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);

                if (null == controllerParent) {
                    controllerParent = ((ViewGroup) controller.getParent());
                }

                removeView(contentFrame);
                controllerParent.removeView(controller);
                contentView.addView(contentFrame, new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
                contentView.addView(controller);
            }

            if (null != displayModeListener) {
                displayModeListener.onDisplayModeChanged(displayMode);
            }
        }
    }

    public void setDisplayModeListener(MxPlayerControlView.DisplayModeListener listener) {
