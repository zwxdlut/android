...
public class MxPlayerViewUtil {
    ...
    public static String formatTime(long milliseconds) {
        ...
        long totalSeconds = Math.round((float) milliseconds / 1000);
        ...
        try {
            mFormatter = new Formatter(stringBuilder, Locale.getDefault());
            if (hours > 0) {
    ...
    public static SimpleCache getVideoCache(Context context) {
        if (videoCache == null) videoCache = new SimpleCache(new File(context.getExternalCacheDir(), "exo_player"), new NoOpCacheEvictor());
        return videoCache;
    }
    ...
    public static void savePlayPosition(Context context, String url, long position) {
        context.getSharedPreferences("MX_VIDEO_PALYER_PLAY_POSITION",
    ...
    public static long getSavedPlayPosition(Context context, String url) {
        return context.getSharedPreferences("MX_VIDEO_PALYER_PLAY_POSITION",
                Context.MODE_PRIVATE)
                .getLong(url, 0);
    }
